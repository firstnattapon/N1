    ...
    def  trade (self):
        while True:
            ...
            if (dataset.Predict[-1] == True) & (diff < -1.00) :
                ...
                st.write( dataset.Predict[-1] , 'Buy' , round(diff , 2), round(a , 2) , round(p , 2))

            elif (dataset.Predict[-1] == False) & (diff > 1.00) :
                deribit.create_market_sell_order(self.pair_trade , abs(diff))
                st.write( dataset.Predict[-1] , 'Sell' , round(diff , 2) , round(a , 2) , round(p , 2))
            else:
                st.write( dataset.Predict[-1] , 'Wait' , round(diff , 2) , round(a , 2) , round(p , 2))
